...
class BoardService(
    ...
    fun save(boardRequest: BoardRequest): Mono<BoardResponse> {
        boardRequest.createdDate = boardRequest.createdDate
     ...
     */
    @Cacheable(cacheNames = ["boards"], key = "#pageable.pageNumber")
    fun getAll(pageable: Pageable): Flux<BoardResponse> {
        return try {
            boardRedisRepository.findAll(pageable)
